...
namespace Probe.Controllers
{
    public class HomeController : Controller
    {
            ...
            try
            {
                ...
                foreach (string typ in types)
                {
                    ...
                    if(typ=="Capital"){
                        ...
                        sb.Append("<textarea class=\"budgetTextArea\" disabled style=\"overflow:hidden;text-align:left;color:black!important;height:35px;\">" + edittyp + "</textarea>");
                    ...
                    else if (typ == "Expense")
                    {
                        ...
                        edittyp = "Current Budgeted Expense E to P";
                        sb.Append("<textarea class=\"budgetTextArea\" disabled style=\"overflow:hidden;text-align:left;color:black!important;height:60px;\">" + typ + " \nOriginal \nApproved</textarea>"); 
                        sb.Append("<textarea class=\"budgetTextArea\" disabled style=\"overflow:hidden;text-align:left;color:black!important;height:35px;\">" + edittyp + "</textarea>");
                        sb.Append("<textarea class=\"budgetTextArea\" disabled style=\"overflow:hidden;text-align:left;color:black!important;\">" + "Projected \n" + typ + "</textarea>");
                        sb.Append("<textarea class=\"budgetTextArea\" disabled style=\"overflow:hidden;text-align:left;color:black!important;\">" + "Actual \n" + typ + "</textarea>");
                        ...
                        foreach (string typ in types)
                        {
                            ...
                            if (propos != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    string newTxtBx1 = "<span style='float:left;width:90px;margin: 0px 5px 0px 5px;'>" + amt + "</span>";
                                    sb.Append(newTxtBx1);
                ...
                foreach (BudgetViewRecord rcd in Details)
                {
                    ...
                    sbTotal.Append(newTxtBx + rcd.Budgeted.ToString() + "\" id=\"" + id + "\" disabled/>");
                    id = rcd.BudgetType + "Projected";
                    sbTotal.Append(newTxtBx + rcd.Projected.ToString() + "\" id=\"" + id + "\" disabled/>");
                    id = rcd.BudgetType + "Actual";
                    sbTotal.Append(newTxtBx + rcd.Actual.ToString() + "\" id=\"" + id + "\" disabled/>");
                    sbTotal.Append("<br/><br/>"); 
                }
        ...
        public void AddBenefit(int ProjectId, int BenefitId, string BenefitName)
        {
            ...
            {
                usr = (ProbeUser)Session["CurrProbeUser"];
                data.AddProjectBenefit(ProjectId, BenefitId, usr.UserName);
        ...
        public string GetHtmlForProjectBenefits(int ProjectId)
        {
            ...
            List<ProjectCostSaving> savings = data.GetProjectSavings(ProjectId);
            List<ProjectBenefit> benefits = data.GetProjectBenefits(ProjectId);
            ...
            sb.Append("<div class=\"ui-corner-all colWrap3\">");
            foreach (string group in grps)
            {
                sb.Append("<div class=\"dontsplit\"><h5>" + group + "</h5>");
                foreach(ProjectBenefit benefit in benefits)
                {
                    if (benefit.BenefitGroupName == group)
                    {
                        if (benefit.Selected == 0)
                            sb.Append(newChkBx + benefit.BenefitId + "\" onclick=\"javascript:UpdateBenefit(" + ProjectId + ", " + benefit.BenefitId + ", '" + benefit.BenefitName + "', this)\" />  " + benefit.BenefitName + "<br />");
                        else
                            sb.Append(newChkBx + benefit.BenefitId + "\" onclick=\"javascript:UpdateBenefit(" + ProjectId + ", " + benefit.BenefitId + ", '" + benefit.BenefitName + "', this)\" checked />  " + benefit.BenefitName + "<br />");
                        }
                    }
                sb.Append("</div>");
            }
        ...
        private string UserHasAccess(int ProjectId)
        {
            ...
            if (proj.ApprovalStatus == "Proposed")
            {
                foreach (string pr in usr.ProjectRoleList)
                {
                    if (pr == "Sponsor" || pr == "Manager" || pr == "Engineer" || pr == "Creator")
                        HasAccess = "yes";
                }
            }
            else
        ...
        public ActionResult NewProject(NewProj proj)
        {
            ...
            try
            {
                ...
                {
                    //Save to Projects
                    ret = data.AddProjectTwoOutputs(proj.ProjectName, proj.BudgetNum, proj.Scope, proj.Comments, proj.Customer, proj.ProjectType, proj.StartDate, proj.EndDate, proj.updateBy, 0, 0, proj.ServiceAgreement, proj.ExpirationDate);
                    int projId = ret[0];
                    int usrid = ret[1];
        ...
        public ActionResult SelectedProject(string id, string SelectedTab)
        {
            ...
            try
            {
                ...
                ProjectInfo selProject = data.GetProjectInfo(Convert.ToInt32(Session["CurrProjectId"].ToString()));
        ...
        public ActionResult ApprovedProjectInfo()
        {
            ...
            try
            {
                ...
                selProject.spank=data.getTotalBudgetCost(pid);
        ...
        public JsonResult UpdateProjectData(ProjectInfo mod)
        {
            ...
            try
            {
                ...
                if (mod.warrantyAgreementDate != null)
                {
                    dt = Convert.ToDateTime(mod.warrantyAgreementDate);
                    h = data.UpdateProjectRecord(mod.ProjectId, mod.warrantyAgreement, usr.UserName, oldModel.Scope, oldModel.Comments, oldModel.ProjectType, oldModel.ProposedStart, oldModel.Customer, oldModel.ProposedEnd, dt, usr.UserId);
                    
                }
                else
                {
                    
                    h = data.UpdateProjectRecord(mod.ProjectId, mod.warrantyAgreement, usr.UserName, oldModel.Scope, oldModel.Comments, oldModel.ProjectType, oldModel.ProposedStart, oldModel.Customer, oldModel.ProposedEnd, null, usr.UserId);
                }
        ...
        [HttpPost]
        public ActionResult addAttachment(int projectId, IEnumerable<HttpPostedFileBase> uploadFile)
        {
            try
            {                
                usr = (ProbeUser)Session["CurrProbeUser"];
                if(uploadFile.Count() > 0)
                {
                    foreach (var ufile in uploadFile)
                    {
                        if (ufile != null)
                        {
                            ...
                            int? newAttachmentId = -1;

                            string shortfilName = ufile.FileName.Substring(ufile.FileName.LastIndexOf("\\")+1);

                            int ret = data.addProjectAttachment(projectId, shortfilName, bits, filetype, usr.UserName, ref newAttachmentId);
        ...
        public ProjectInfo PopulateProjectInfoLists(ProjectInfo project)
        {
            ...
            budTypes.Add(new SelectListItem { Value = "Capital", Text = "Capital" });
            budTypes.Add(new SelectListItem { Value = "Expense", Text = "Expense" });
